...
public class BookingService
{
    ...
    public async Task<ServiceResponse> GetAllBookingsAsync()
    {
        try
        {
            ...
            _logger.LogInformation($"Successfully fetched {bookings?.Count ?? 0} bookings");
            return ServiceResponse.Success(bookings ?? new List<BookingResponseDto>(), "Bookings loaded successfully");
        }
    ...
    public async Task<ServiceResponse> UpdateBookingAsync(int id, UpdateBookingDto updateDto)
    {
        try
        {
            ...
            var updatedBooking = await response.Content.ReadFromJsonAsync<BookingResponseDto>();
    ...
    public async Task<ServiceResponse> GetBookingsByDateAsync(DateTime date)
    {
        try
        {
            ...
            var response = await _httpClient.GetAsync($"api/Booking/by-date?date={dateString}");
            if (!response.IsSuccessStatusCode)
                return ServiceResponse.Failure($"API returned {response.StatusCode}: {response.ReasonPhrase}");
